...
芯片在输入框工具行**右侧**，紧挨官方模型选择器。只显示窗口数字（如 `256k`），悬停才是「上下文 256k」，避免把超长模型名挤到第二行。官方模型 / 思考强度弹层不变。

打开后，从上到下：

1. **上下文** — `128k` / `256k` / `392k` / `512k` / `1M`。默认写给**当前对话模型、全局生效**（会镜像进 `llm-pi-ai`）。勾 **仅当前会话** 则只覆盖这一次对话，不改目录。选 `1M` 会提示：部分模型在该长度上会额外计费（没有按模型查价）。
...
| 字段 | 含义 |
| --- | --- |
| `contextWindows` | `供应商/模型` → 128000 / 256000 / 392000 / 512000 / 1000000 |
